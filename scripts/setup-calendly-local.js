...
async function setupCalendlyLocal() {
  ...
  try {
    ...
    console.log(
      '  (Run "ngrok http 3000" in another terminal and copy the HTTPS URL)'
    );
    console.log("");

    const ngrokUrl = await question(
      "Enter ngrok HTTPS URL (e.g., https://abc123.ngrok.io): "
    );
    ...
    try {
      const testResponse = execSync(`curl -s ${webhookUrl}/test`, {
        encoding: "utf8",
      });
      ...
      console.log(
        "Make sure your development server is running and ngrok is forwarding correctly"
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.log(
        "1. Calendly OAuth set up (run: npm run setup-calendly-oauth)"
      );
    ...
    console.log(
      "  ngrok http 3000               # Start ngrok (in another terminal)"
    );
