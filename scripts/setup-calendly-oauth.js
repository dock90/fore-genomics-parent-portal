...
const server = http.createServer((req, res) => {
  ...
  if (error) {
    ...
    res.end('<h1>Authorization Failed</h1><p>Check the console for details.</p>');
    server.close();
    rl.close();
    return;
  }
  ...
  if (code) {
    ...
    exchangeCodeForToken(code)
      .then(tokenData => {
        ...
        console.log(`CALENDLY_TOKEN_EXPIRES_AT=${new Date(Date.now() + tokenData.expires_in * 1000).toISOString()}`);
        console.log('');
        console.log('🔧 You may also want to set up token refresh for production use.');
        
        res.writeHead(200, { 'Content-Type': 'text/html' });
        res.end('<h1>✅ Authorization Successful!</h1><p>Check the console for your access token.</p>');
      })
      .catch(err => {
