...
export default function MultiKitOnboardingForm({
  ...
  const [completedKits, setCompletedKits] = React.useState<Set<string>>(
    new Set()
  );
  ...
  React.useEffect(() => {
    ...
    if (
      activeKitIndex >= 0 &&
      isKitCompleted(activeKitIndex) &&
      activeKitIndex < kits.length - 1
    ) {
      // Find next incomplete kit
      const nextIncompleteIndex = childrenData.findIndex(
        (_, index) => index > activeKitIndex && !isKitCompleted(index)
      );
  ...
  const [celebratingKit, setCelebratingKit] = React.useState<string | null>(
    null
  );
  ...
  >(new Map());
  const [successStates, setSuccessStates] = React.useState<
    Map<string, boolean>
  >(new Map());
  ...
  const validateKitCompletion = (
    kitIndex: number
  ): { isValid: boolean; missingFields: string[]; completionScore: number } => {
    const childData = childrenData[kitIndex];
    if (!childData) {
      return {
        isValid: false,
        missingFields: ["Kit data not found"],
        completionScore: 0,
      };
    }
    ...
    if (!childData.childInfo) {
      ...
      if (childInfo.ethnicity && childInfo.ethnicity.length > 0)
        childInfoScore += 8;
    ...
    if (typeof childData.questionnaire.question2 === "boolean")
      questionnaireScore += 10;
    if (typeof childData.questionnaire.question3 === "boolean")
      questionnaireScore += 10;
    ...
    switch (section) {
      case "childInfo":
        if (!childData.childInfo) {
          ...
          if (!childInfo.ethnicity || childInfo.ethnicity.length === 0)
            errors.push("Ethnicity selection is required");
          if (!childInfo.relationshipToChild)
      ...
      case "consent":
        ...
        const consentValidation = validateKitSectionWithData(
          kitIndex,
          "consent",
          childData
        );
      ...
      case "questionnaire":
        if (childData.questionnaire.question1 === undefined)
          errors.push("Question 1 must be answered");
        if (childData.questionnaire.question2 === undefined)
          errors.push("Question 2 must be answered");
        if (childData.questionnaire.question3 === undefined)
          errors.push("Question 3 must be answered");
  ...
  const validateKitSectionRealTime = (
    kitIndex: number,
    section: "childInfo" | "consent" | "questionnaire"
  ) => {
    console.log(
  ...
  const validateKitSectionWithData = (
    kitIndex: number,
    section: "childInfo" | "consent" | "questionnaire",
    childData: any
  ) => {
    ...
    switch (section) {
      case "childInfo":
        ...
        if (!childData.childInfo) {
          ...
          if (!childInfo.ethnicity || childInfo.ethnicity.length === 0)
            errors.push("Ethnicity selection is required");
          if (!childData.childInfo.relationshipToChild)
            errors.push("Relationship to child is required");
          if (childInfo.dob) {
            const dob = new Date(childInfo.dob);
            const today = new Date();
  ...
  const validateKitRealTime = (kitIndex: number) => {
    const childInfoValidation = validateKitSectionRealTime(
      kitIndex,
      "childInfo"
    );
    const consentValidation = validateKitSectionRealTime(kitIndex, "consent");
    const questionnaireValidation = validateKitSectionRealTime(
      kitIndex,
      "questionnaire"
    );

    const isValid =
      childInfoValidation.isValid &&
  ...
  const validateKitRealTimeWithData = (
    kitIndex: number,
    updatedChildData: any
  ) => {
    // Use the provided data instead of reading from state
    const childInfoValidation = validateKitSectionWithData(
      kitIndex,
      "childInfo",
      updatedChildData
    );
    ...
    const questionnaireValidation = validateKitSectionWithData(
      kitIndex,
      "questionnaire",
      updatedChildData
    );
    ...
    kits.forEach((kit, index) => {
      ...
      if (!validation.isValid) {
        ...
        summary.errorSummary.push(
          `Kit ${index + 1}: ${validation.errors.join(", ")}`
        );
      }
    });
  ...
  const getKitCompletionDetails = (kitIndex: number) => {
    ...
    if (!childData)
      return { status: "error", message: "Kit data not found", score: 0 };
  ...
  const handleKitCompletion = (kitIndex: number) => {
    ...
    setTimeout(() => {
      ...
      const nextIncompleteIndex = childrenData.findIndex(
        (_, index) => index > kitIndex && !isKitCompleted(index)
      );
      ...
      if (nextIncompleteIndex !== -1) {
        ...
        setTimeout(() => {
          const nextKitElement = document.querySelector(
            `[data-kit-id="${kits[nextIncompleteIndex].id}"]`
          );
          if (nextKitElement) {
  ...
  const loadCompletionState = (kitId: string) => {
    try {
      ...
      if (stored) {
        ...
        const now = new Date();
        const daysDiff =
          (now.getTime() - storedTime.getTime()) / (1000 * 60 * 60 * 24);
  ...
  React.useEffect(() => {
    ...
    const loadAllCompletionStates = () => {
      const loadedCompletions = new Map<
        string,
        {
          completedAt: Date;
          completedBy: string;
  ...
  const syncFormStateAcrossPanels = (
    kitIndex: number,
    section: "childInfo" | "consent" | "questionnaire"
  ) => {
    const childData = childrenData[kitIndex];
  ...
  const validateAllDataBeforeCompletion = () => {
    ...
    return {
      ...
      summary: {
        ...
        totalErrors: invalidKits.reduce(
          (sum, kit) => sum + kit.errors.length,
          0
        ),
      },
    };
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      if (isMountedRef.current) {
        const hasDirtyForms = childrenData.some(
          (childData) => childData.isDirty
        );
        if (hasDirtyForms) {
          autoSaveFormData();
        }
  ...
  const handleCompleteOnboarding = async () => {
    ...
    if (!validationResult.isValid) {
      const errorMessage = `Please fix the following issues before submitting:\n${validationResult.invalidKits
        .map((kit) => `Kit ${kit.kitIndex + 1}: ${kit.errors.join(", ")}`)
        .join("\n")}`;
    ...
    try {
      ...
      for (let i = 0; i < childrenData.length; i++) {
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to save onboarding data for kit ${childData.kitId}`
          );
        }
          ...
          <div className="mt-6">
            ...
            <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden shadow-inner">
              <div
                ...
                style={{
                  width: `${(completedKits.size / kits.length) * 100}%`,
                }}
              >
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center gap-3 text-blue-800">
            ...
            <span className="font-medium">
              Processing your onboarding data...
            </span>
          </div>
        </div>
      )}
        ...
        <>
              ...
              <div className="bg-white rounded-2xl p-8 shadow-2xl border-4 border-blue-400 animate-pulse max-w-md w-full max-h-[90vh] overflow-y-auto">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-bold text-gray-900 mb-2">
                    Kit {kits.find((k) => k.id === celebratingKit)?.kitNumber}{" "}
                    Completed!
                  </h3>
                  <p className="text-gray-600 mb-4">
                    {(() => {
                      const kitIndex = kits.findIndex(
                        (k) => k.id === celebratingKit
                      );
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-lg font-bold text-blue-800">
                        {(() => {
                          ...
                          return kitIndex >= 0
                            ? getKitCompletionDetails(kitIndex).score
                            : 100;
                        })()}
                        %
                      </span>
                ...
                >
                  ...
                  <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
                    ...
                    <ChildInfoStep
                      ...
                      onSave={(values: ChildInfo) =>
                        handleChildInfoSubmit(index, values)
                      }
                    ...
                    {childrenData[index]?.childInfo ? (
                      ...
                      <>
                        ...
                        <ConsentStep
                          ...
                          existingConsentData={
                            childrenData[index]?.consentData || null
                          }
                          ...
                          isActive={activeKitIndex === index}
                          saving={
                            loadingStates.get(`${kit.id}-consent`) || false
                          }
                    ...
                    childrenData[index]?.consentAccepted ? (
                      ...
                      <>
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div className="ml-auto flex items-center gap-2">
                            {/* Show completion status for this section */}
                            {childrenData[index]?.questionnaire.question1 !==
                              undefined &&
                              childrenData[index]?.questionnaire.question2 !==
                                undefined &&
                              childrenData[index]?.questionnaire.question3 !==
                                undefined && (
                                <Badge variant="default" className="text-xs">
                                  ✓ Completed
                                </Badge>
                        ...
                        <QuestionnaireStep
                          questionnaire={
                            childrenData[index]?.questionnaire || {
                              question1: undefined,
                              question1Details: "",
                              question2: undefined,
                              question2Details: "",
          ...
          <div className="flex justify-center mt-10">
            <Button
              ...
              disabled={
                saving || completedKits.size !== kits.length || globalLoading
              }
              size="lg"
              className={`
          ...
          {saveError && (
            <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
              ...
              <div className="mt-2 text-sm text-red-700">
                Please review the errors above and try again. If the problem
