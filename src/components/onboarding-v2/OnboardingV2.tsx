...
export function OnboardingV2({ user, orderId, initialData }: OnboardingV2Props) {
  ...
  const handleComplete = useCallback(async (state: OnboardingState) => {
    // TODO: Save to API
  ...
  // Handle next button click - delegates to step's internal submit
  const handleNext = useCallback(() => {
    // Dispatch custom event that step components listen to
  ...
  const hideNavigation = currentStep?.id === 'confirmation' || currentStep?.id === 'share-prompt';
